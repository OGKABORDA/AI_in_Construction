...
def count_letters(main_str):
    main_str = main_str.lower() # т.к. регистр считается одинаково - приведем все в нижний
    letter_count = {}
    for char in main_str:
        if char.isalpha(): #проверка на то, что символ явл. буквой
            letter_count[char] = letter_count.get(char, 0) + 1   #увеличиваем счетчик значений(кол-во), если ключ(буква) совпадает
    return letter_count
